...
class RequestLogStore:
    ...
    def _is_token_invalid_or_expired_error(item: dict) -> bool:
        message = str(item.get("error") or item.get("message") or "").strip().casefold()
        return "token invalid or expired" in message

    @staticmethod
    def _has_token_identity(item: dict) -> bool:
        if not isinstance(item, dict):
            return False
        return any(
            str(item.get(key) or "").strip()
            for key in ("token_id", "token_account_email", "token_account_name")
        )
    ...
    def find_poll_invalid_token_candidates(self, *, limit: int = 500) -> dict:
        ...
        for item in deduped_items:
            ...
            if not self._is_token_invalid_backfill_candidate(item):
                continue
            if not self._has_token_identity(item):
                unidentified_logs += 1
                continue
...
class ErrorDetailStore:
    ...
    @staticmethod
    def _is_token_invalid_or_expired_error(item: dict) -> bool:
        message = str(item.get("message") or item.get("error") or "").strip().casefold()
        return "token invalid or expired" in message
    ...
    def find_invalid_token_candidates(self, *, limit: int = 500) -> dict:
        ...
        deduped_items = [*latest_by_code.values(), *anonymous_items]
        grouped: dict[str, dict] = {}
        matched_logs = 0
        unidentified_logs = 0
